...
// Returns nil if the version is empty or the model is not found (no filtering).
func ResolveModelVersionToID(ctx context.Context, deps *FilterLookupDeps, version string) (*uint, error) {
	...
	if err != nil {
		// Model not found is not an error for filtering — just means no matches
		return nil, nil //nolint:nilerr // intentional: missing model means no results to filter
	}
